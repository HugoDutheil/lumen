...
# Shift + Cmd + 7 to comment/uncomment
...
def test_single_head(test_data, test_labels, compounds, model, threshold=1e-1):
    ...
    model.load_state_dict(torch.load("../weights/model_weights_single_head.pth", map_location="cpu"))
    model.eval()
    with torch.no_grad():
    ...
    sample = 10
    print(f"Sample {sample} Ground Truth")
    print(sample_y[sample])
    print("============================================")
    print(f"Sample {sample} Predicted Components")
